    ...
    try {
        ...
        if (userDid && authToken) {
            try {
                ...
                // const telegramData = getTelegramData(userDid, authToken);
        ...
        let twitterData= userData;
        let telegramGroups= telegramData.groups || 0;
        let telegramMessages= telegramData.messages || 0;;

        const totalScore = evaluateUser(twitterData, walletData, telegramGroups, telegramMessages)
...
function calculateScore(twitterData, walletData, telegramGroups, telegramMessages) {
    ...
    const socialScore = (
        ...
        (twitter.retweet_count || 0) * weights.retweets +  // NEW: Retweets
        (twitter.quote_count || 0) * weights.quotes +     // NEW: Quotes
        (twitter.reply_count || 0) * weights.replies      // NEW: Replies
    );

    // Crypto/DeFi Activity Score (max 40)
    const cryptoScore = (
        ...
        wallet.transactionCount * weights.transactionCount +              // NEW: Transaction count
        wallet.uniqueTokenInteractions * weights.uniqueTokenInteractions  // NEW: Unique token interactions
    );

    // NFT Engagement Score (max 30)
    const nftScore = wallet["Wallet NFTs Result"].length * weights.nftHoldings;

    // Community Engagement Score (max 20)
    const communityScore = (
        twitterData.result.creator_subscriptions_count * weights.subscriptions +
        telegram.length * weights.groupCount
    );

    // Telegram Activity Score (max 15)
    const telegramScore = (
        telegram.length * weights.groupCount +
        ...
        messages.filter(m => m.sourceData.content._ === "messageSticker").length * weights.stickerMessages +  // NEW: Sticker messages
        messages.filter(m => m.sourceData.content._ === "messageAnimation").length * weights.gifMessages +    // NEW: GIF messages
        messages.reduce((sum, m) => sum + (m.sourceData.content.entities || []).filter(e => e.type._ === "textEntityTypeMention").length, 0) * weights.mentionCount  // NEW: Mentions
    );

    // Total Score with caps
